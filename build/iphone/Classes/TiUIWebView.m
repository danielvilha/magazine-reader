...
static NSString * const kMagazineReaderJavascript = @"Ti.App={};Ti.API={};Ti.App._listeners={};Ti.App._listener_id=1;Ti.App.id=Ti.appId;Ti.App._xhr=XMLHttpRequest;"
...
- (NSString *)_magazinereaderInjection
{
	if (pageToken==nil) {
	...
	[html appendString:kMagazineReaderJavascript];
...
+ (NSString *)content:(NSString *)content withInjection:(NSString *)injection
{
	...
	// attempt to make well-formed HTML and inject in our MagazineReader bridge code
	   ...
	   baseURL:(NSURL*)baseURL
{
	...
	content = [[self class] content:content withInjection:[self _magazinereaderInjection]];
...
- (BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType
{
	...
	if ([scheme hasPrefix:@"http"] || [scheme isEqualToString:@"ftp"]
		...
		if ([scheme isEqualToString:@"file"] || [scheme isEqualToString:@"app"]) {
			[NSURLProtocol setProperty:[self _magazinereaderInjection] forKey:kContentInjection inRequest:(NSMutableURLRequest *)request];
		}
		return YES;
	}
